...
public:
    ...
    void Left(Node * root, vector < int > & res) {
      Node * cur = root -> left;
      while (cur) {
        if (!isLeafNode(cur)) res.push_back(cur -> data);
...
void Right(Node * root, vector < int > & res) {
  ...
  vector < int > temp;
  while (cur) {
    if (!isLeafNode(cur)) temp.push_back(cur -> data);
    if (cur -> right) cur = cur -> right;
    else cur = cur -> left;
  }
  
  //reverse array beacuse we move downward to upward for boundary traversal
  for (int i = temp.size() - 1; i >= 0; --i) {
    ...
    vector <int> boundary(Node *root)
    {
      ...
      //first left boundary-->leaf nodes-->right boundary
      Left(root,res);
